...
export const getAvailableCoupons = (orderInfo) => {
  ...
  return validCoupons.filter((coupon) => {
    ...
    if (
      ...
      orderInfo.totalAmount < coupon.minOrderAmount
    ) {
      return false;
    }
    ...
    switch (coupon.scope) {
      ...
      case COUPON_SCOPES.NEW_USER:
        // 检查是否为新用户
        return (
          userState.createTime &&
          Date.now() - userState.createTime < 7 * 24 * 60 * 60 * 1000
        ); // 7天内视为新用户
...
export const calculateDiscount = (coupon, orderInfo) => {
  ...
  switch (coupon.type) {
    case COUPON_TYPES.DISCOUNT:
      // 折扣券，如8折
      return parseFloat(
        (orderInfo.totalAmount * (1 - coupon.value / 10)).toFixed(2)
      );
    ...
    case COUPON_TYPES.FREE:
      ...
      return coupon.value > 0
        ? Math.min(coupon.value, orderInfo.totalAmount)
