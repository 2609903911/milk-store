      ...
      <view class="order-item" v-for="(order, index) in orders" :key="index">
        ...
        <view class="order-content" @tap="viewOrderDetail(order)">
          ...
          <view class="order-info">
            ...
            <view class="panda-coins" v-if="order.orderStatus !== 'cancelled'">
              <text class="panda-coins-text"
                >获得熊猫币: {{ getPandaCoins(order.totalAmount || 0) }}</text
              >
            </view>
          </view>
          <view class="order-price">
            <view class="price-tag">¥{{ order.actualAmount }}</view>
            <view class="price-detail">共{{ getTotalQuantity(order) }}件</view>
            <view
        ...
        <view class="order-footer">
          ...
          <view
            ...
            >取消订单</view
          >
          <view class="action-btn delete-btn" @tap="deleteOrder(index)"
            >删除订单</view
          >
        </view>
      </view>
