<template>
  <view class="container">
    ...
    <view class="notice-bar">
      <!-- #ifdef H5 -->
      <uni-icons type="notification" size="18" color="#ff9500"></uni-icons>
      <!-- #endif -->
      <!-- #ifdef MP-WEIXIN -->
      <text
              ...
              >
                ...
                <view class="product-info">
                  ...
                  <view class="product-price-container">
                    ...
                    <view
                      class="add-to-cart"
                      @click.stop="openProductDetail(category, product)"
                      >+
                    </view>
                  </view>
                </view>
              ...
              >
                ...
                >
                  <view class="discount-value"
                    >{{ coupon.discount
                    }}<text class="discount-unit">{{ coupon.unit }}</text></view
                  >
    ...
    if (userData.coupons && Array.isArray(userData.coupons)) {
      ...
      coupons.value = userData.coupons
        .map((coupon) => {
          ...
          const couponType = coupon.couponTemplate?.type || coupon.type;
          const couponValue = coupon.couponTemplate?.value || coupon.value;
          const couponTitle = coupon.couponTemplate?.title || coupon.title;
          const couponDesc =
            coupon.couponTemplate?.description || coupon.description;
          const couponScope = coupon.couponTemplate?.scope || coupon.scope;
          const minOrderAmount =
            coupon.couponTemplate?.minOrderAmount || coupon.minOrderAmount || 0;
          ...
          } else if (coupon.createTime && coupon.couponTemplate?.validDays) {
            // 如果有创建时间和有效天数，计算过期时间
          ...
          return {
            ...
            description:
              couponDesc ||
              (minOrderAmount > 0 ? `满${minOrderAmount}元可用` : ""),
            expireDate,
            scope: couponDesc,
            color,
