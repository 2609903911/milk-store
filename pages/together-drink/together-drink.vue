  ...
  <view class="together-drink-container">
    ...
    <view class="delivery-area">
      ...
      <view class="delivery-options">
        ...
        >
          <uni-icons
            :type="deliveryType === 'self' ? 'checkbox-filled' : 'circle'"
            size="20"
        ...
        >
          <uni-icons
            :type="deliveryType === 'delivery' ? 'checkbox-filled' : 'circle'"
            size="20"
            :color="deliveryType === 'delivery' ? '#1890ff' : '#999'"
          ></uni-icons>
          <text>外卖配送</text>
        </view>
      </view>
      ...
      <view class="address-info">
        ...
        <view class="store-address">
          ...
          <view class="address-content">
            ...
            <view class="store-full-address">{{ storeAddress.address }}</view>
            <view class="store-distance" v-if="storeAddress.distance"
      ...
      <view class="participants-box">
        ...
        <view class="participant-item">
          <image
            ...
            mode="aspectFill"
          ></image>
          <view class="participant-name">{{ userInfo.nickname || "我" }}</view>
        ...
        <view
          ...
          v-if="
            state === 'waiting' || state === 'initial' || state === 'created'
          "
        >
          <view class="empty-avatar">
            <uni-icons type="contact" size="30" color="#CCCCCC"></uni-icons>
          </view>
    ...
    <view class="action-area">
      ...
      >
        <view class="code-display">
          ...
          <button class="copy-button" @tap="copyInviteCode">复制</button>
        </view>
        <view class="countdown">
          <text class="countdown-text">有效期：{{ formatCountdown }}</text>
      ...
      <view class="ready-actions" v-if="state === 'ready'">
        ...
        <button class="action-button primary pay-button" @tap="payOrder">
          立即支付
        </button>
    ...
    <view class="instruction-area">
      ...
      <view class="instruction-content">
        <text class="instruction-item">1. 邀请好友一起喝，共享欢乐时光</text>
  ...
  try {
    ...
    if (invitation.creatorInfo) {
      partnerInfo.value = {
        ...
        avatar:
          invitation.creatorInfo.avatar || "/static/images/avatar-default.png",
      };
    ...
    try {
      const joinResponse = await joinInvitation(invitationId.value, joinData);

      // 检查响应状态
      if (joinResponse.data && joinResponse.data.code === 500) {
        toast(joinResponse.data.message || "邀请不存在或已不可加入", "none");
        return;
      }
...
const initAddressInfo = () => {
  ...
  try {
    ...
    if (storeInfo) {
      const store =
        typeof storeInfo === "string" ? JSON.parse(storeInfo) : storeInfo;
      storeAddress.value = {
        id: store.id || "",
        name: store.name || "",
  ...
  try {
    ...
    if (invitationData.productId) {
      ...
      product.value = {
        ...
        imageUrl:
          invitationData.productInfo.productImage || product.value.imageUrl,
      };
    }
  } catch (error) {
    // 捕获错误但不中断流程
        ...
        if (response && response.data && typeof response.data === "object") {
          ...
          if (userData.data && userData.data.nickname) {
            partnerInfo.value = {
              ...
              avatar:
                userData.data.avatar || "/static/images/avatar-default.png",
            };
